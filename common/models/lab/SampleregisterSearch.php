       ...
       $query->joinWith(['request']);
        ...
        if (count($params) == 0)
        {
           ...
           $labId = (int) Yii::$app->request->get('lab_id');
            ...
            $query->where('status_id < :statusId AND lab_id = :labId AND active =:active AND request_ref_num != "" AND status_id = 1 AND DATE_FORMAT(`request_datetime`, "%Y-%m-%d") BETWEEN :fromRequestDate AND :toRequestDate',[':statusId'=>2,':labId'=>$labId,':active'=>1,':fromRequestDate'=>$fromDate,':toRequestDate'=>$toDate]);
            $query->orderBy([
                'tbl_sample.sample_code' => SORT_ASC,
            ]);
        }
