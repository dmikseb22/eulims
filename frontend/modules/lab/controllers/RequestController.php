...
//notification for cashier and accountant
...
class RequestController extends Controller
{
    ...
    { 
        ...
        //get overdues transaction
        $todaydues = Request::find()
                    ->leftJoin('tbl_testreport', 'tbl_request.request_id=tbl_testreport.request_id')
                    ->where(['report_due'=>date('Y-m-d')])
                    // ->andWhere(['like','request_datetime',date('Y-m-')])
                    ->andWhere(['tbl_testreport.report_num' => null])
                    ->andWhere(['not',['tbl_request.request_ref_num' => null]])
                    ->andWhere(['not',['tbl_request.status_id' => 0]])
                    ->andWhere(['not',['tbl_request.lab_id' => 3]])
                    ->count();

        //get overdues transaction
        $overdues = Request::find()
                    ->leftJoin('tbl_testreport', 'tbl_request.request_id=tbl_testreport.request_id')
                    ->where(['<','report_due',date('Y-m-d')])
                    ->andWhere(['like','request_datetime',date('Y-')])
                    ->andWhere(['tbl_testreport.report_num' => null])
                    ->andWhere(['not',['tbl_request.request_ref_num' => null]])
                    ->andWhere(['not',['tbl_request.status_id' => 0]])
                    ->andWhere(['not',['tbl_request.lab_id' => 3]])
        ...
        //get the unpaids with payment status id of 1
        $unpaids = Request::find()
                    ->select(['total'=>'sum(total)'])
                    ->where(['like','request_datetime',date('Y-m-')])
                    ->andWhere(['not',['request_ref_num' => null]])
                    ->andWhere(['not',['status_id' => 0]])
                    ->andWhere(['payment_status_id' => 1])
                    ->andWhere(['payment_type_id' => 1])
                    ->all();

        //get the unfinished transaction
        $unfinished = Request::find()
                    ->where(['request_ref_num' => null])
                    ->andWhere(['not',['status_id' => 0]])
                    ->count();

        //get the partial payment
        $partials = Request::find()
                    ->where(['like','request_datetime',date('Y-m-')])
                    ->andWhere(['not',['request_ref_num' => null]])
                    ->andWhere(['not',['status_id' => 0]])
                    ->andWhere(['payment_status_id' => 3])
                    ->andWhere(['payment_type_id' => 1])
                    ->count();
    ...
    public function actionMore($topic){
        $title = "";
        switch ($topic) {
            case 'unfinished':
                $title = "Unfinished Requests";
                $query = exRequest::find()
                    ->where(['request_ref_num' => null])
                    ->andWhere(['not',['status_id' => 0]]);
                break;
            ...
            case 'overdues':
                ...
                $query = exRequest::find()
                    ...
                    ->andWhere(['not',['tbl_request.status_id' => 0]])
                    ->andWhere(['not',['tbl_request.lab_id' => 3]])
                    ->orderBy(['tbl_request.request_datetime'=>'DESC']);
                break;

            case 'todaydues':
                $title = "Requests Due Today";
                $query = exRequest::find()
                    ->leftJoin('tbl_testreport','tbl_request.request_id=tbl_testreport.request_id ')
                    ->where(['report_due'=>date('Y-m-d')])
                    // ->andWhere(['like','request_datetime', date('Y-m-')])
    ...
    public function actionPaymentstatus($id)
    {
       ...
       $paymentitem = Paymentitem::find()->select('orderofpayment_id')->where(['request_id' => $request->request_id])->andWhere(['not', ['orderofpayment_id' => 0]])->andWhere(['not', ['receipt_id' => null]])->all();
       if ($paymentitem){
            $Receipt = Receipt::find()->where(['in', 'orderofpayment_id',$paymentitem])->andWhere(['<>','cancelled','1']);
        }else{
            //how about searching receipt that have no OP yet;
            $no_op = paymentitem::find()->where(['request_id' => $request->request_id])->andWhere(['not', ['receipt_id' => null]])->andWhere(['not', ['cancelled' => 0]])->all();
            if(!$no_op){
                return "No Receipt Issued!";
            }

            $text = "These are receipt/s that have no OP";
            foreach ($no_op as $no_op_item) {
                $text.=" <br/> ".$no_op_item->details." - Amount of ".$pitem->amount;
            }
...
"https://chart.googleapis.com/chart?cht=qr&chl='.$sample['sample_code'].'&chs=65x65&chld=L|0"
    ...
    public function actionUploadform($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            $proof= UploadedFile::getInstance($model, 'proof');

            if(!empty($proof) && $proof->size !== 0) {
                $proof->saveAs('uploads/request/'.$model->request_ref_num.".".$proof->extension);
                $model->proof='uploads/request/'.$model->request_ref_num.".".$proof->extension;
            }


            if($model->save(false)){
                Yii::$app->session->setFlash('success', 'PDF copy was successfully added!');

                $profile= Profile::find()->where(['user_id'=> Yii::$app->user->id])->one();
                //check notification if already created
                $newnotif  = OPNotifcation::find()->where(['request_id'=>$model->request_id])->one();
                if(!$newnotif){
                    //add notification for the accountant to generate OP
                    $newnotif = new OPNotifcation; //reinitiate
                }
                $newnotif->request_id =$model->request_id;
                $newnotif->sender_id=Yii::$app->user->id;
                $newnotif->sender_name=$profile->firstname.' '. strtoupper(substr($profile->middleinitial,0,1)).'. '.$profile->lastname;
                $newnotif->notification_date=date('Y-m-d');
                $newnotif->save();

            }else{
                Yii::$app->session->setFlash('error', 'PDF copy was NOT uploaded!');
            }
            
            return $this->redirect(['/lab/request/view?id='.$id]);
        }

        // return $model->request_id;
       if(Yii::$app->request->isAjax){
                return $this->renderAjax('_uploadform', [
                    'model'=>$model
               ]);
       }
    ...
    public function actionDownloadform($id)
    {
       $model = $this->findModel($id);
        ...
        return $this->renderAjax('_downloadform', [
            'model' => $model,
        ]);
    ...
    /**
     * Displays a single Request model without any controls.
     * @param integer $id
     * @return mixed
     */
    public function actionViewtransaction($id)
    {
        ...
        //this id is the test report number
        $testreport = Testreport::find()->where(['report_num' => $id])->one();

        if(!$testreport)
            throw new \yii\web\NotFoundHttpException("Test Report Number not found");

        $sample_ids = '';
        foreach ($testreport->testreportSamples as $sample){
            $sample_ids .= $sample->sample_id.",";
        }
        $sample_ids = substr($sample_ids, 0, strlen($sample_ids)-1);
        ...
        $samplesQuery = Sample::find()->WHERE(['in', 'sample_id',$ids]);
        $sampleDataProvider = new ActiveDataProvider([
            'query' => $samplesQuery,
            'pagination' =>false,
        ]);
        ...
        $model=exRequest::findOne($testreport->request_id);


        return $this->render('viewtransaction', [
            // 'model' => $model,
            'model'=>$testreport,
            'sampleDataProvider' => $sampleDataProvider,
            'analysisdataprovider'=> $analysisdataprovider,
