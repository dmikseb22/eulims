    ...
    <div class="container table-responsive">
        <?php
            ...
            echo GridView::widget([
                ...
                'hover'=>true,
                'panel' => [
                    'heading'=>'<h3 class="panel-title"> <i class="glyphicon glyphicon-file"></i> QR Verification Label, Samples and Test Results</h3>',
                    'type'=>'primary',
                    'before'=> ($request->payment_status_id==2 || $request->payment_type_id == 2)?Html::button('<i class="glyphicon glyphicon-print"></i> Download Targeted QR Label', ['disabled'=>!$enableRequest, 'onclick'=>"window.location.href = '" . \Yii::$app->urlManager->createUrl(['/reports/lab/testreport/printlabel2','request_id'=>$model->testreport_id]) . "';" ,'title'=>'Used to re-print test report',  'class' => 'btn btn-success'])." ".Html::button('<i class="glyphicon glyphicon-print"></i> Download QR Label', ['disabled'=>!$enableRequest, 'onclick'=>"window.location.href = '" . \Yii::$app->urlManager->createUrl(['/reports/lab/testreport/printlabel3','request_id'=>$model->testreport_id]) . "';" ,'title'=>'Used to be inserted in the test report',  'class' => 'btn btn-success'])." or <img src='https://chart.googleapis.com/chart?cht=qr&chl=".Url::base(true).Url::toRoute(['/lab/request/viewtransaction', 'id' =>$model->report_num])."&chs=80x80&chld=L|0'/> copy this image" :Html::button('<i class="glyphicon glyphicon-print"></i> Print QR Verification Label', ['disabled'=>true,'title'=>'Print Label',  'class' => 'btn btn-warning']).'<br/><p class="text-danger"> QR can only be downloaded when the request has been paid or is fully subsidized</p>',
                        'after'=>false,
                ],
                'columns' => $gridColumns,
