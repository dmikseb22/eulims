...
class AccomplishmentController extends \yii\web\Controller
{
	...
	**/
    ...
    public function actionIndex()
    {
		...
		if (Yii::$app->request->get())
		{
            ...
            ->select([
                ...
                'total'=>'SUM(total)',
                'request_datetime'=>'AN_VALUE(request_datetime)',
            ...
            ->where('rstl_id =:rstlId AND status_id > :statusId AND lab_id = :labId AND DATE_FORMAT(`request_datetime`, "%Y") = :year AND DATE_FORMAT(`request_datetime`, "%m") = :month AND request_ref_num != "" AND request_type_id = 1', [':rstlId'=>$rstlId,':statusId'=>0,':labId'=>$labId,':year'=>$year,':month'=>$month])
            ->groupBy(['yearmonth'])
            ->orderBy('request_datetime ASC');
		}
    ...
    public function actionFirms(){
        ...
        ->where('rstl_id =:rstlId AND status_id > :statusId AND DATE_FORMAT(`request_datetime`, "%Y") = :year AND request_ref_num != "" AND lab_id=:lab_id', [':rstlId'=>$rstlId,':statusId'=>0,':year'=>$year,':lab_id'=>Yii::$app->user->identity->profile->lab_id])
        ->groupBy(['month'])
        ...
        $labname = Lab::findOne(Yii::$app->user->identity->profile->lab_id);
        if (Yii::$app->request->isAjax) {
    ...
    public function actionShows($monthyear,$type=null){
        $reqs =  Requestextend::find()->where(['DATE_FORMAT(`request_datetime`, "%Y-%m")' => $monthyear])->andWhere(['>','status_id',0])->leftJoin('tbl_customer', 'tbl_request.customer_id=tbl_customer.customer_id')->andWhere(['tbl_customer.classification_id'=>$type,'lab_id'=>Yii::$app->user->identity->profile->lab_id]);
         $dataProvider = new ActiveDataProvider([
            'query' => $reqs,
            'pagination' => false,
        ]);
