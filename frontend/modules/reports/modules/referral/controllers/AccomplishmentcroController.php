...
namespace frontend\modules\reports\modules\referral\controllers;
    ...
    public function actionIndex()
    {
		...
		if($report_type==1){
			$modelReferral = Requestextension::find()
				->select([
					'monthnum'=>'DATE_FORMAT(ANY_VALUE(`request_datetime`), "%m")',
					'month'=>'DATE_FORMAT(ANY_VALUE(`request_datetime`), "%M")',
					'totalrequests' => 'count(request_id)',
					'request_datetime'=>'ANY_VALUE(request_datetime)',
                ...
                ->groupBy(['yearmonth'])
                ->orderBy([
                    'DATE_FORMAT(ANY_VALUE(request_datetime), "%Y")' => SORT_DESC,
                    'DATE_FORMAT(ANY_VALUE(request_datetime), "%m")' => SORT_ASC,
                ]);
