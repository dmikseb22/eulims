...
class AccomplishmentController extends \yii\web\Controller
{
	...
	public function actionIndex(){
        ...
        if (Yii::$app->request->post()) {
            $data = Yii::$app->request->post('Srchref');
        ...
        $result = $oldreferralcomp->referralreport($model);

        $dataprovider = new ArrayDataProvider([
                    'allModels' => isset($result->dataprovider)?$result->dataprovider: null,
                    'pagination' => false,
                ]);
	...
	public function actionReferralfigure($type,$ids,$rstl_id){
		try {
			$oldreferralcomp = new OldreferralComponent;
			return $model = $oldreferralcomp->referralfigures($type,$ids,$rstl_id);	
		} catch (Exception $e) {
			return "error";
		}
	}
